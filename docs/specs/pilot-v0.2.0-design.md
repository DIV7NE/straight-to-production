...
│   ├── plugin.json
│   └── marketplace.json
├── commands/                    # 6 commands (down from 10)
...
|-----------|--------|--------|-----|
| Commands | 10 | 6 | Removed: milestone (merged into evaluate), clean (Opus can do this), standards (just read CLAUDE.md), upgrade (re-run setup) |
...
### /pilot:feature — Autonomous Feature Builder

**Purpose:** Build a feature with full autonomy, only interrupting for product decisions. Teach key concepts along the way.

**Allowed tools:** Read, Write, Bash, Glob, Grep, AskUserQuestion

**Interaction model:** Opus builds autonomously. Only asks the user about PRODUCT decisions (branding, business logic, user-facing choices). Never asks about implementation approach. Explains key technical concepts at decision points so the user learns.

**Process:**

#### Step 1: Understand Context

Read CLAUDE.md for project spec, stack, and patterns. Check if `.pilot/current-feature.md` already exists (in-progress feature).

#### Step 2: Enrich (0-1 product questions max)

Based on the feature description, identify what standards and concerns are relevant. Read the matching `.pilot/references/` files.

Ask at most ONE question if a product decision is needed:
...
If no product decision is needed, skip straight to the plan.
